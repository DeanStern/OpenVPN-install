...
cat /etc/openvpn/openvpn.log | grep "CLIENT_LIST,trialUser" | awk -F',' -v now=`date +%s` '{print "$SERVER_NICK_NAME Trial",$3,$6,$7,now-$9}' >> /root/openVPN-udp/OpenVPN.log
cat /etc/openvpn/openvpn.log | grep "CLIENT_LIST,vpnuser" | awk -F',' -v now=`date +%s` '{print "$SERVER_NICK_NAME",$3,$6,$7,now-$9}' >> /root/openVPN-udp/OpenVPN.log
